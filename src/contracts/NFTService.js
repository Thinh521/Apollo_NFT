...
class NFTServiceClass extends BaseContractService {
   ...
   */
  async approveNFT(walletProvider, tokenId) {
    const signer = await this._getSigner(walletProvider);
    const contract = this._connect(walletProvider, signer);

    try {
      const marketplace = CONTRACT_ADDRESSES.MARKETPLACE_ADDRESS;

      const approvedAddress = await contract.getApproved(tokenId);

      if (approvedAddress.toLowerCase() === marketplace.toLowerCase()) {
        return {success: true, alreadyApproved: true};
      }

      showMessage({
        message: 'Vui lòng xác nhận approve trong ví',
        type: 'info',
      });

      const gas = await contract.approve.estimateGas(marketplace, tokenId);

      const result = await this.sendTx(contract.approve, marketplace, tokenId, {
